...
int main (int argc, char *argv [] ) {
  ...
  socklen_t s_in_size;
  s_in_size = sizeof(adresseClient);
...
/* attachement de la socket a l'adresse locale */
  if ((bind(socket_RV, (const struct sockaddr *)&adresseRV, lgadresseRV)) == -1){
      perror("bind");
      exit(3);}
...
/* boucle d'attente de connexion */
  while(TRUE){
      ...
      socket_service=accept(socket_RV, (struct sockaddr *)&adresseClient, &s_in_size);
